...
local function request(opts, callback)
	...
	vim.system(args, { text = true }, function(obj)
		if obj.code ~= 0 then
			vim.notify("[notion.nvim] Request failed", vim.log.levels.ERROR)
			return callback(nil)
		end
		callback(obj.stdout)
	end)
...
local function fetch_pages(api_key, callback)
	request({
		...
		if not ok then
			vim.notify("[notion.nvim] Invalid JSON response", vim.log.levels.ERROR)
			return
		end
		local pages = {}
		for _, page in ipairs(decoded.results or {}) do
			local title = page.properties
				and page.properties.title
				and page.properties.title.title
				and page.properties.title.title[1]
				and page.properties.title.title[1].plain_text
			if title then
...
function M.list(api_key)
	if not api_key then
		vim.notify("[notion.nvim] Missing api key", vim.log.levels.ERROR)
		return
	end
